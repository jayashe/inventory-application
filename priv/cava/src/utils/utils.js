...
export function fetchUnits(options) {
  const { token } = options;
  const url = `${baseApiUrl}/units`;
  return fetchData(url, 'units of measurement');
}
...
export function fetchInventory(options) {
  ...
  return fetchData(url, 'inventory');
}

function fetchData(url, entityType) {
  return fetch(url, {
    headers: {
  ...
  .catch(error => {
    console.error('Could not fetch ' + entityType, error);
  });
}
...
export function createUnit(options) {
  const { token, unit } = options;
  const url = `${baseApiUrl}/units?token=${token}`;
  ...
  .catch(error => {
    console.log('Could not create unit', error);
  });
}
